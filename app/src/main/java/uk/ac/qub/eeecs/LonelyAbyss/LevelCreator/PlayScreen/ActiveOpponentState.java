...
public class ActiveOpponentState extends State {
    ...
    //ReleaseButton opponentButton;


    private UnimonCard activeUnimonCard1; //the unimon card that the player has currently active

    private GameObject playerButton; // the button that will show the opponent's screen
    ...
    public ActiveOpponentState(ScreenViewport mScreenViewport, LayerViewport mLayerViewPort, Game mGame, GameScreen mGameScreen, Boolean active, BattleSetup battleSetup) {
    ...
    public void initialiseCards(BattleSetup battleSetup) {
        activeUnimonCard1 = mGame.getAssetManager().getUnimonCards().get(1);
    ...
    public void updateCards(ElapsedTime elapsedTime) {
        ...
        activeUnimonCard1.update(elapsedTime);
    }
    ...
    public void touchActiveUnimon(TouchEvent t) {
        if ((activeUnimonCard1.getBound().contains((int) t.x, (int) mLayerViewPort.getTop() - t.y))) {
            this.touchActive = false;
            playScreen.getActiveUnimonState().active = true;
        }
    }

    //checks whether the player button has been touched
    public void touchPlayerButton(TouchEvent t) {
        if ((playerButton.getBound().contains((int) t.x, (int) mLayerViewPort.getTop() - t.y))) {
            //J Devlin 40150554
            this.touchActive = false;
            playScreen.getPlayOverviewState().active = true;
            playScreen.getActiveOpponentState().active = false;
        }
    }
...
//        }
    ...
    public void drawCards(ElapsedTime elapsedTime, IGraphics2D graphics2D) {
        ...
        activeUnimonCard1.draw(elapsedTime, graphics2D, mLayerViewPort, mScreenViewport);
    }
    ...
    public void generateActiveUnimon() {
        ...
        activeUnimonCard1.setPosition(mGameScreen, x, y, width, height);
    }
    ...
    public void generateShowOpponentsScreenButton() {
        ...
        playerButton = new GameObject(x, y, width, height, selectBitmap("Opponent"), mGameScreen);
    }
  ...
  */
    ...
    public UnimonCard getActiveUnimonCard() {
        return activeUnimonCard1;
    }

    public void setActiveUnimonCard(UnimonCard activeUnimonCard) {
        this.activeUnimonCard1 = activeUnimonCard;
    }
